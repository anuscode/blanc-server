...
from model.models import Alarm, Setting, User


def push(data: dict = None, user: User = None, priority="normal"):
    ...
    try:
        ...
        progress(message=message, data=data, user=user)
        messaging.send(message)
    except Exception as e:
        logging.exception(e)


def progress(message: messaging.Message, data: dict = None, user: User = None):
    if is_event(of=Alarm.Event.LOG_OUT, data=data):
        log_out(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.APPROVED, data=data):
        approved(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.REJECTED, data=data):
        rejected(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.BLOCKED, data=data):
        blocked(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.CONVERSATION, data=data):
        conversation(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.POKE, data=data):
        poke(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.REQUEST, data=data):
        request(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.COMMENT, data=data):
        comment(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.POST_FAVORITE, data=data):
        favorite(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.MATCHED, data=data):
        matched(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.COMMENT_THUMB_UP, data=data):
        thumb_up(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.CONVERSATION_OPEN, data=data):
        conversation_open(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.CONVERSATION_LEAVE, data=data):
        conversation_leave(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.LOOK_UP, data=data):
        look_up(message=message, data=data, user=user)
        return

    if is_event(of=Alarm.Event.STAR_RATING, data=data):
        star_rating(message=message, data=data, user=user)
        return
...
def poke(message: messaging.Message, data: dict = None, user: User = None):
...
def request(message: messaging.Message, data: dict = None, user: User = None):
...
def comment(message: messaging.Message, data: dict = None, user: User = None):
...
def favorite(message: messaging.Message, data: dict = None, user: User = None):
...
def matched(message: messaging.Message, data: dict = None, user: User = None):
...
def thumb_up(message: messaging.Message, data: dict = None, user: User = None):
...
def conversation_open(message: messaging.Message, data: dict = None, user: User = None):
...
def conversation_leave(message: messaging.Message, data: dict = None, user: User = None):
    if not is_event(of=Alarm.Event.CONVERSATION_LEAVE, data=data):
        pass


def look_up(message: messaging.Message, data: dict = None, user: User = None):
...
def star_rating(message: messaging.Message, data: dict = None, user: User = None):
